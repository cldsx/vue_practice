...
var elToTemplate = function (el) {
  ...
  for (var index = 0; index < el.childNodes.length; index++) {
    // 排除掉一些特殊的标签和截图工具本身
    if (
      ["SCRIPT", "#comment", "STYLE", "NOSCRIPT"].indexOf(
        el.childNodes[index].nodeName
      ) > -1 ||
      (el.childNodes[index].getAttribute &&
        el.childNodes[index].getAttribute("page-view") == "snipping-tool")
    ) {
      continue;
    }
    ...
    // 节点
      ...
      if (el.childNodes[index].nodeName == "IMG") {
        ...
        template +=
          "<img src='" + keyValue + "' style='" + styleTemplate + "'/>";
      } else {
